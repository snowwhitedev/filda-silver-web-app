    ...
    const getVotedCount = async (pollsDataArray) => {
        ...
        for (const poll of pollsDataArray) {
            if (poll.proposalType && '1' === poll.proposalType) {
                ...
                const proposalContract = await VoteData.getMultiProposalContract(web3, address)
                const [poolSnapshot, proposalOptions] = await Promise.all([
                    proposalContract.methods.poolSnapshot().call(),
                    VoteData.getMultipleVoteOptions(web3, networkType, address)
                ])
                const options = []
                for (const p of proposalOptions) {
                    let i = proposalOptions.indexOf(p)
                    ...
                    const ps = new BigNumber(poolSnapshot).div(1e18).toFixed(0).toString(10)
                    options.push({title: p, totalVoted: Number(value), poolSnapshot: Number(ps)})
                }

                proposals.push({address, proposalOptions: [...options]})
            }
        }
        return proposals
    }
        ...
        async function initialLoad() {
            ...
            if (web3) {
                ...
                const filDaDaoContractAddress = Object.entries(Config.pools)
                  .filter(pool => pool[1].lpTokenName === 'FilDA').map(pool => pool[0]).join('')
                                        ...
                                        .map((item) => {
                                          return item.map((val, idx) => (
                                            <div className={styles.multiVoteContainer} key={idx}>
                                                <RangeSlider
                                                  option={{title: val.title, value: val.totalVoted}}
                                                  max={val.poolSnapshot}
                                                  readonly={true}
                                                />
                                            </div>
                                          ))
