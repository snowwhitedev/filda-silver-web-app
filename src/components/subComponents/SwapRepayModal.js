...
import LoadingIcon from '../../assets/images/loanloading.svg'
import LoadingSpinner from '../../assets/images/loadingspin.svg'
import CheckIcon from '../../assets/images/checkyellow.svg'
...
export default function SwapRepayModal(props) {
    ...
    useEffect(() => {
        if (props.show === true) {
            const swapPairs = {
                "MDX": ["HUSDT", "HBTC", "ETH", "HT", "BEE", "CAN"],
                "HT": ["HBTC", "ETH", "HUSD"],
                "HBTC": ["ETH"],
                "HUSDT": ["HBTC", "HT", "HUSD", "ETH", "HLTC", "HBCH", "HDOT", "HFIL", "HPT", "LHB", "AAVE", "SNX", "UNI", "LINK", "BAL", "YFI", "MKR", "COMP", "SLNV2"],
                "HUSD": ["FILDA"]
            }
    ...
    const newSwapRouter = () => {
        return new SwapRouter(web3, getTokenAddress('HUSDT'), getTokenAddress('HUSD'), Config.WHT, Config.MDEXRouter);
    }
    ...
    const onSwapValueUpdate = async(value) => {
        ...
        setSwapValue(value)
        if (numberFromString(value) === 0) {
            setRepayValue('')
            return
        }

        setCalculating(true)
        const txnValue = await getRawValue(swapAsset.symbol, numberFromString(value))
    ...
    const RepayButton = 
        (swapPath.length === 0 || calculating || overPay || isNaN(parseFloat(swapValue)) || isNaN(parseFloat(repayValue)) || parseFloat(swapValue) <= 0 || parseFloat(repayValue) <= 0 )?
        ...
        const ModalRepayCustom = () =>
            <div>
                <Modal.Body>
                    ...
                    {overPay ? <div className={styles.txnError}>{t('RepayModal.OverPayError')}</div> : ''}
                    {noSwapRoute ? <div className={styles.txnError}>{t('SwapRepayModal.NoSwapRouteError')}</div> : ''}
                    {largeDeviation ? <div className={styles.txnError}>{t('SwapRepayModal.LargeDeviationError')}</div> : ''}
                    <div className={styles.footerInfo}>
        ...
        <div>
            <Modal.Body>
                ...
                <Form.Check
                    ...
                    onClick={() => {setCheckedDeviation(v => !v)}}
