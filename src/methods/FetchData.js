...
const getPriceInUSD = async(web3, networkType, market) => {
    var usdtMarket = Config.markets['USDT']
    if (!usdtMarket.qToken.network[networkType]) usdtMarket = Config.markets['HUSD']
    if (market.symbol == 'USDT' || market.symbol == 'HUSD') {
        return 1
    }

    const contractABI = Config.priceOracle.ABI
