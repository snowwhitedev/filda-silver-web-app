...
const getPollsDataArray = async(web3, networkType, connectedAddress) => {
    const singleAddresses = Config.voteProposal.network[networkType].single
    const proposalPromises = []
    for (const singleAddress of singleAddresses) {
        proposalPromises.push(getProposalInfo(web3, connectedAddress, singleAddress))
    }
    const multiAddresses = Config.voteProposal.network[networkType].multiple
    for (const address of multiAddresses) {
        proposalPromises.push(getProposalInfo(web3, connectedAddress, address, true))
    }
...
const getProposalContract = async(web3, networkType) => {
    ...
    const singleVoteLength = Config.voteProposal.network[networkType].single.length
    const proposalAddress = Config.voteProposal.network[networkType].single[singleVoteLength - 1]
